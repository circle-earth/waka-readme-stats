...
async def get_waka_time_stats(repositories: Dict, commit_dates: Dict) -> str:
    ...
    if EM.SHOW_TIMEZONE or EM.SHOW_LANGUAGE or EM.SHOW_EDITORS or EM.SHOW_PROJECTS or EM.SHOW_OS:
        ...
        stats += f'<tr><th colspan="1" width="400" align="center"> 📊 {FM.t("This Week I Spend My Time On")} </th></tr>'

        if EM.SHOW_TIMEZONE:
            DBM.i("Adding user timezone info...")
            time_zone = data["data"]["timezone"]
            stats += f'<tr><th colspan="1" width="400" align="center"> 🕑︎ {FM.t("Timezone")}: {time_zone} </th></tr>'
        ...
        if EM.SHOW_PROJECTS:
            ...
            project_list = no_activity if len(data["data"]["projects"]) == 0 else make_list(data["data"]["projects"], title=FM.t('Projects'))
            stats += f"{project_list}\n\n"
